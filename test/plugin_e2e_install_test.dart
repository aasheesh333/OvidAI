...
void main() {
  ...
  final pluginRoot = Directory('/tmp/opencode/superpowers');
  ...
  test('installs the real plugin and mounts its skills for a session',
      () async {
    if (!pluginRoot.existsSync()) {
      markTestSkipped('superpowers checkout not present');
      return;
    ...
    expect(result!.status, isNot(PluginInstallStatus.failed),
        reason: result.error ?? '');
  ...
  test('the real plugin SessionStart hook produces injectable context',
      () async {
    if (!pluginRoot.existsSync()) {
      markTestSkipped('superpowers checkout not present');
      return;
